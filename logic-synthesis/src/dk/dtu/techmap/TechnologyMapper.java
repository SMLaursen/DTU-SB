...
public class TechnologyMapper {
     ...
     * Starts mapping.
     * 
     * @return The set of parts in the solution. Empty for no solution.
     */
    public void start() {
        solutions.clear();
        HashMap<String, LogicGate> startingGate = new HashMap<String, LogicGate>();
        startingGate.put(graph.getOutputGate().getProtein(),
                graph.getOutputGate());
     ...
     * 
     * @return empty : no match could be found Set : the set of parts that make
     *         up the match
     */
		...
		for(String protein : toMatch.keySet()){
			...
			for(SBGate sbGate : Library.getGatesWithOutput(protein)){
				...
				if(allSelectedParts.contains(sbGate) ||
				   isProteinsOverLapping(sbGate, allSelectedParts))
				   {
				    continue;
				}
    ...
    /**
     * Checks whether proteins from new gate already exists
     */
    private boolean isProteinsOverLapping(SBGate newGate, HashSet<SBGate> allSelectedParts) {
        HashSet<String> allUsedProteins = new HashSet<String>();
        for (SBGate gate : allSelectedParts) {
            allUsedProteins.addAll(gate.intermediateProteins);
            allUsedProteins.addAll(gate.inputProteins);
        }
        HashSet<String> newGateProteins = new HashSet<String>(newGate.inputProteins);
        newGateProteins.addAll(newGate.intermediateProteins);
        for (String newProtein : newGateProteins) {
            if (allUsedProteins.contains(newProtein)) {
                return true;
            }
        }
        return false;
     ...
     * 
     * @return empty list : incompatible library part. non-empty list :
     *         compatible library part where If list-map is empty finished
     *         matching. if list-map is non-empty further matching necessary.
     * */
    public LinkedList<HashMap<String, LogicGate>> isMatching(
            LogicGate otherPart, LogicGate thisPartPos) {
        ...
        if (otherPart instanceof AndGate && thisPartPos instanceof AndGate) {
            ...
            for (HashMap<String, LogicGate> nextMap1 : isMatching(otherChild1,
                    thisChild1)) {
                for (HashMap<String, LogicGate> nextMap2 : isMatching(
                        otherChild2, thisChild2)) {
            ...
            // If type signature match and protein are specified in the inMap of
            // the AIG
            if (thisPartPos instanceof InputGate
                    && graph.containsInputProtein(((InputGate) otherPart)
                            .getProtein())) {
                // Ensure proteins really match
                if (((InputGate) otherPart).getProtein().equals(
                        ((InputGate) thisPartPos).getProtein())) {
                    // nextMatches are empty here
                    set.add(nextMatches);
                }
            } else {
                // Continue matching from thisPartPos node using other's protein
                nextMatches.put(((InputGate) otherPart).getProtein(),
                        thisPartPos);
                set.add(nextMatches);
            }
            return set;
